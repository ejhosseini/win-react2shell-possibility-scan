...
.DESCRIPTION
    ...
    It checks:
      1. Installed JS runtimes / CLI: node, bun, deno, pm2
         - via PATH (Get-Command)
         - via registry for Node.js (Apps & Features)
      2. Running processes: node.exe, bun.exe, deno.exe, pm2.exe
      ...
      4. Listening ports that often indicate Next.js-style apps
...
function Get-JsRuntimeInfo {
    ...
    # 1a. Detect CLI in PATH
    $runtimeNames = @("node", "bun", "deno", "pm2")

    $installedCli = foreach ($name in $runtimeNames) {
    ...
    # 1b. Detect Node.js in registry (Apps & Features)
    $nodeReg = Get-NodeInstallFromRegistry
    $installedReg = foreach ($n in $nodeReg) {
        [PSCustomObject]@{
            Runtime        = "node"
            Source         = "Registry"
    ...
    if ($runtimeProcs) {
        ...
        $runtimeProcs | Format-Table -AutoSize
    } else {
...
function Get-RscFileIndicators {
        ...
        foreach ($pkg in $pkgFiles) {
            ...
            foreach ($d in $deps) {
                switch -Wildcard ($d.Name) {
                    ...
                    "react-server-dom-*" {
                        $hasRsc = $true
                        $rscPackages += ("{0}={1}" -f $d.Name, $d.Value)
                    }
                    "waku" {
                        $hasRsc = $true
                        $rscPackages += ("{0}={1}" -f $d.Name, $d.Value)
            ...
            if ($hasNext -or $hasRsc -or $hasNextBuild -or $hasTextRsc) {
                ...
                elseif ($hasNextBuild -and $hasNext) {
                    $risk = "Medium (Next and .next - verify App Router and versions)"
                }
                elseif ($hasNext) {
    ...
    if ($results.Count -gt 0) {
        Write-Host ""
        Write-Host "Potential RSC / Next.js apps detected:" -ForegroundColor Yellow
        $results |
...
# --------------------------------------------------------------------
# 3) Local listening ports - Next.js-style
# --------------------------------------------------------------------
...
# --------------------------------------------------------------------
# 4) Remote connections to other servers on Next-like ports
# --------------------------------------------------------------------
...
$hasRuntime = ($runtimeInfo.InstalledRuntimes.Count -gt 0 -or $runtimeInfo.RuntimeProcesses.Count -gt 0)
$hasRscApp  = ($fileIndicators | Where-Object { $_.HasRscPackages -or $_.TextRscHitPath }).Count -gt 0
$hasPort    = ($localPorts.Count -gt 0 -or $remoteConns.Count -gt 0)

if ($hasRuntime -or $hasRscApp -or $hasPort) {
